...
Note - Using: VS Code Thema oh-lucy and font Cascadia Code
...
├─/dev/nvme0n2p2
└─/dev/nvme0n2p3
/dev/nvme0n3 #<- This is new disk added, with no file system.
```


You can label the disk with only one of the option which is gpt, msdos
Parted command is provide non-interactive option i.e. in a few lines of code you can create partitions
...
Below I have used variable expansion (i love bash)
...
File system type?  [ext2]? linux-swap # <-- If you do not remember this name, you can use help mkpart
Start? 2048s
...
Now, mount this new swap partition. Lets check what is current swap size
...
Note: The current swap size is 2.0Gi. Now lets mount the partition

You can also assign priority to swap file using `pri` in fstab. 
...
# lets check the swap priority
[preetam@rhelgui ~]$ sudo swapon --show
